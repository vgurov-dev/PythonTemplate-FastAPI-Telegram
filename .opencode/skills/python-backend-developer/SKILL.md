...
- **ID — только int, не UUID**
  - ✅ Хорошо: `id: Optional[int] = Field(default=None, primary_key=True)`
  - ❌ Плохо: `id: UUID = Field(default_factory=uuid4, primary_key=True)`
  - Причина: UUID усложняет поиск, индексы, дебаг. Для внутренних PK достаточно auto-increment.
...
- **Анализировать структуру папок перед Docker**
  - ✅ Хорошо: Проверить названия — нет ли конфликтов (app vs application)
  - ❌ Плохо: Создать папку с именем, которое конфликтует с другой папкой

- **Именовать папки без конфликтов**
  - ✅ Хорошо: `bootstrap/` + `application/` (разные смыслы)
  - ❌ Плохо: `app/` + `application/` (почти одно и то же)

- **Volumes — максимально простые**
  - ✅ Хорошо: Один volume `./src/backend/:/app/` покрывает всё
  - ❌ Плохо: 9 отдельных volumes для каждой папки

- **Не дублировать файлы**
  - ✅ Хорошо: pyproject.toml в корне, монтировать в volume
  - ❌ Плохо: Копировать pyproject.toml в src/backend/ (нужно синхрить)

- **Понимать порядок сборки Docker**
  1. Dockerfile COPY копирует в image при build
  2. Volume монтирует поверх при run
  3. Для development: volume важнее (обновляется в реальном времени)

- **Особенности macOS + Docker**
  - Volume mountpoint может выйти за пределы rootfs
  - Решение: включать файл в Docker context (COPY), не в volume